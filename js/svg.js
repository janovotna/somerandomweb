...
var svgcolorize = "yellow";
function getsvgcolor(){svgcolorize = "yellow";}
var coordX = [0,0,0,0,1,1,1,1,1,2];
var svgcolorX = coordX[Math.floor(Math.random()*coordX.length)];

if(localStorage["focusses"]){
	newfocus = localStorage.getItem("focusses");
	newfocus = 	parseInt(newfocus) + 1;
	localStorage.setItem('focusses', newfocus);

}else{
	localStorage.setItem('focusses', 1);
};

var factor1 = Math.floor(Math.random() * ((6-14)+1) + 14);
if(localStorage.getItem("focusses") == factor1 || easter_egg_value){
	$("#getin").append("Gratulujeme!");
	$(".resulz").append("Podařilo se Vám zodpovědět 3 ze 3 otázek správně!");
  $(".tweetz").attr('data-title', 'Podařilo se mi zodpovědět 3/3 otázek správně a dostat se tak na očkování proti COVIDu-19!!!!');
  
  // set entry video
  // update subtitles
  question4 = entry;
  $('#main4 source').attr('src', entry.sauce);
  $("#main4")[0].load();
  $("#main4").get(0).play();

}else{
	$("#getin").append("Sorry jako, ale dnes vakcína nebude.");
	$(".resulz").append("Podařilo se Vám správně zodpovědět "+ svgcolorX +" ze 3 otázek.");
	$(".tweetz").attr('data-title', 'Podařilo se mi zodpovědět '+ svgcolorX + '/3 otázek správně a na vakcínu si tak budu muset ještě počkat...');
}
